...
\item ¿El resultado se pide módulo algo, porque el valor exacto tendría del orden de \ten{17} dígitos?
...
\graybold{Complejidad:} \bigO{\log n} multiplicaciones modulares: tres por cada bit de $n$, que tiene a lo sumo 60 bits para $n \le \ten{18}$.

\graybold{Fundamento teórico:} De la identidad de Fibonacci $F_{m+k} = F_m F_{k+1} + F_{m-1} F_k$ se derivan, tomando $m = k$ y $m = k+1$, las dos fórmulas de duplicación:
\[F_{2k} = F_k\,(2F_{k+1} - F_k), \qquad F_{2k+1} = F_k^2 + F_{k+1}^2.\]
Ambas expresan el par $(F_{2k}, F_{2k+1})$ en función del par $(F_k, F_{k+1})$, así que manteniendo siempre un par consecutivo se puede duplicar el índice con tres multiplicaciones. Para llegar a un $n$ arbitrario se recorren sus bits de mayor a menor empezando en $(F_0, F_1) = (0,1)$: en cada paso se duplica el índice acumulado y, si el bit vale 1, se avanza además un término, pasando de $(F_{2k}, F_{2k+1})$ a $(F_{2k+1}, F_{2k+2})$. Esto es exponenciación binaria disfrazada: el índice se construye bit a bit igual que el exponente en una potenciación rápida. Como todo se hace módulo $\ten{9}+7$, las magnitudes se mantienen chicas; la única sutileza es que $2F_{k+1} - F_k$ puede quedar negativo tras reducir, y el operador módulo de Python ya devuelve un representante no negativo, así que no hace falta corregirlo a mano (en C++ sí haría falta). La alternativa clásica es elevar a la potencia $n$ la matriz de $2\times2$ cuya primera fila es $(1,\,1)$ y cuya segunda fila es $(1,\,0)$: da el mismo resultado con hasta dieciséis multiplicaciones por bit (dos productos de matrices de ocho cada uno) en lugar de tres, pero tiene la ventaja de generalizarse a cualquier recurrencia lineal cambiando la matriz.~\cite{knuth1997}

\graybold{Ejercicio aplicado}

(CSES 1722 — Fibonacci Numbers) Dado $n$ con $0 \le n \le \ten{18}$, imprimir $F_n$ módulo $\ten{9}+7$.

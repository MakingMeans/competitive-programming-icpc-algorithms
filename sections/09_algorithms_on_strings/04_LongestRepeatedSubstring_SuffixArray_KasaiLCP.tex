...
\graybold{Extras: qué subcadena es y dónde aparece} — variantes de \texttt{solve()} para cuando el problema pide, además de la longitud, QUÉ subcadena es y DÓNDE aparece (\texttt{lcp\_detailed}: todas las subcadenas de longitud máxima con sus posiciones) o cuál es la subcadena más larga que aparece el MAYOR número de veces (\texttt{lcp\_repeated}, con mínimo en ventana deslizante sobre el arreglo LCP). Se llaman con \texttt{(s, sa, lcp, n)} en lugar de \texttt{max(lcp)}.

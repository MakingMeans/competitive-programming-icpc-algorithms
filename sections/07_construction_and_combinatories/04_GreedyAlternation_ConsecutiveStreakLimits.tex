\subsection{Construcción Greedy por Alternancia (Límites de Racha Consecutiva)}
...
\graybold{¿Para qué sirve?} Construir una secuencia válida de longitud D usando N opciones con límites de racha consecutiva, sin necesidad de una búsqueda ni un algoritmo sofisticado: alternar entre las DOS opciones con mayor límite es siempre suficiente (si el problema es factible), porque cualquier racha se puede "romper" con un solo turno de la segunda opción antes de retomar la primera.
...
\graybold{I/O:} N,D ≤ 10\textsuperscript{5} → lectura total con tokenización (patrón 2). Verificado contra 200 casos aleatorios comparando factibilidad y validando la secuencia construida.

...
\graybold{Cuidado:} \texttt{round} usa redondeo bancario (los empates van al par),
no el "0.5 sube" del colegio, y además arrastra el error del punto flotante:

\begin{lstlisting}[language=Python]
round(0.5)       # 0     (no 1)
round(2.5)       # 2     (no 3)
round(2.675, 2)  # 2.67  (2.675 no es exacto en binario)
int(x + 0.5)     # "0.5 sube" para x >= 0
\end{lstlisting}

Para imprimir con $n$ decimales, \texttt{f"\{x:.2f\}"} es lo habitual y lo que
esperan los jueces.
...
También aceptan más de dos argumentos. Tanto \texttt{math.lcm} como el
\texttt{gcd} de varios argumentos requieren Python 3.9 o posterior; en una versión
anterior se usa \texttt{reduce(math.gcd, lista)} y
\texttt{a * b // math.gcd(a, b)}.
...
Complejidad: \bigO{n} multiplicaciones, pero sobre números cada vez más grandes, así que el tiempo real crece más rápido: con $n = 4\cdot10^5$ ya tarda \aprox{}1.5s.
...
\begin{lstlisting}[language=Python]
7 // 2       # 3
-7 // 2      # -4    (C++ y Java dan -3)
-7 % 2       # 1     (C++ y Java dan -1)
int(-7 / 2)  # -3    truncar hacia cero, como C++
divmod(-7, 2)  # (-4, 1)  cociente y resto de una vez
\end{lstlisting}

Este comportamiento es diferente al de C++ y Java, y tiene una ventaja: \texttt{x \%
m} siempre queda en $[0, m)$ con $m > 0$, así que en aritmética modular no hace
falta sumar $m$ para corregir restos negativos.

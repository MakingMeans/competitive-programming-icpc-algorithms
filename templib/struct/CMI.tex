\subsection{Fenwick Tree con XOR (Nim por Rangos, Sprague-Grundy)}
...
\graybold{¿Para qué sirve?} Generalizar el Fenwick Tree (visto para sumas) a CUALQUIER operación invertible — aquí XOR — para mantener el valor de Grundy combinado de un rango de pilas de Nim, con actualizaciones puntuales en tiempo logarítmico. Combinado con el teorema de Sprague-Grundy/Bouton, responde instantáneamente quién gana un juego de Nim sobre cualquier subrango.
...
\graybold{I/O:} N,Q ≤ 10\textsuperscript{6} → lectura total con tokenización (patrón 2), y las funciones de update/query del Fenwick se escriben SIN llamadas a función (inline) para reducir el overhead de Python en este volumen de operaciones. Aun así, en el peor caso (N=Q=10\textsuperscript{6}) la solución pura en Python ronda \textasciitilde{}7s — si el juez es muy estricto con el límite de tiempo, este es el techo práctico de Python puro para este volumen sin cambiar de lenguaje.
